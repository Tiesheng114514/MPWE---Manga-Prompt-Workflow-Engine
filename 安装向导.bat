...
echo [6/6] 写入配置并收尾 ...
(
    echo # MPWE 项目维护的 ComfyUI 模型路径配置
    echo # 由 安装向导.bat 自动生成；启动 ComfyUI 时传入 --extra-model-paths-config 本文件
...
if /i "!want_key!"=="y" (
    ...
    if defined MPWE_KEY (
        powershell -NoProfile -Command "$k=$env:MPWE_KEY; $p='.env'; $o=@(); $f=$false; Get-Content $p | ForEach-Object { if ($_ -match '^OPENAI_API_KEY=') { $f=$true; $o+='OPENAI_API_KEY='+$k } else { $o+=$_ } }; if (-not $f) { $o+='OPENAI_API_KEY='+$k }; Set-Content $p -Value $o"
    )
    set "TS_SITE="
    set /p "TS_SITE=   请输入 Turnstile Site Key（没有就回车跳过）: "
    if defined TS_SITE (
        set "TS_SECRET="
        set /p "TS_SECRET=   请输入 Turnstile Secret Key: "
        if defined TS_SECRET (
            powershell -NoProfile -Command "$s=$env:TS_SITE; $c=$env:TS_SECRET; $p='.env'; $o=@(); Get-Content $p | ForEach-Object { $o+=$_ }; $o+='TURNSTILE_SITE_KEY='+$s; $o+='TURNSTILE_SECRET_KEY='+$c; Set-Content $p -Value $o"
        )
    )
)
